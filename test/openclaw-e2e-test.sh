...
if api GET /api/openclaw >"$TMP_DIR/oc-status.json" 2>"$TMP_DIR/oc-status.err"; then
  pass "api /api/openclaw reachable"
else
  fail "api /api/openclaw unreachable: $(cat "$TMP_DIR/oc-status.err" 2>/dev/null || true)"
fi

log "T2: status schema fields"
if jq -e '.installed != null and .gatewayRunning != null and .installTaskRunning != null and .repairTaskRunning != null and .gatewayRestartRunning != null' "$TMP_DIR/oc-status.json" >/dev/null 2>&1; then
...
if jq -e '.gatewayWatchdogRunning != null and .operationState != null and (.operationState.type // "") != "" and (.lastBackupAt != null) and (.lastRollbackAt != null)' "$TMP_DIR/oc-status.json" >/dev/null 2>&1; then
...
if api GET /api/openclaw/config/backups >"$TMP_DIR/oc-backups.json" 2>"$TMP_DIR/oc-backups.err" && jq -e '.success == true and (.backups | type == "array")' "$TMP_DIR/oc-backups.json" >/dev/null 2>&1; then
  pass "backups list endpoint ok"
else
  fail "backups list endpoint failed: $(cat "$TMP_DIR/oc-backups.err" 2>/dev/null || true)"
fi

log "T4: restore invalid backup should fail cleanly"
if api POST /api/openclaw/config/restore '{"name":"not-exists.json"}' >"$TMP_DIR/oc-restore-invalid.json" 2>"$TMP_DIR/oc-restore-invalid.err"; then
  fail "restore invalid backup unexpectedly succeeded"
else
  if grep -Eq '404|不存在|invalid|无效' "$TMP_DIR/oc-restore-invalid.err" "$TMP_DIR/oc-restore-invalid.json" 2>/dev/null; then
...
UPDATE_RESP="$(api POST /api/openclaw/update 2>"$TMP_DIR/oc-update.err" || true)"
TASK_ID="$(echo "$UPDATE_RESP" | jq -r '.taskId // ""' 2>/dev/null || true)"
if [ -z "$TASK_ID" ]; then
  fail "update did not return taskId: $(cat "$TMP_DIR/oc-update.err" 2>/dev/null || true) | resp=$UPDATE_RESP"
else
...
if [ -n "$TASK_ID" ]; then
  ...
  sleep 2
  TASK_JSON="$(api GET "/api/openclaw/install/$TASK_ID" 2>"$TMP_DIR/oc-task.err" || true)"
  ...
  if [ "$CHECK_ONLY_COMMAND_CHAIN" != "1" ]; then
    ...
    while [ "$waited" -lt "$TASK_TIMEOUT_SEC" ]; do
      TJSON="$(api GET "/api/openclaw/install/$TASK_ID" 2>"$TMP_DIR/oc-task-poll.err" || true)"
    ...
    if [ "$final_status" = "success" ]; then
      ...
      echo "$TJSON" | jq -r '.log // ""' | tail -n 80 >"$TMP_DIR/oc-task-tail.log"
    else
...
if [ "$CHECK_ONLY_COMMAND_CHAIN" = "1" ]; then
  ...
  if api POST /api/openclaw/start >"$TMP_DIR/oc-start.json" 2>"$TMP_DIR/oc-start.err"; then
    pass "restart api accepted"
  else
    fail "restart api failed: $(cat "$TMP_DIR/oc-start.err" 2>/dev/null || true)"
  fi
...
if api GET /api/openclaw/gateway/logs?lines=120 >"$TMP_DIR/oc-gateway-logs.json" 2>"$TMP_DIR/oc-gateway-logs.err"; then
  logs_text="$(jq -r '.logs // ""' "$TMP_DIR/oc-gateway-logs.json" 2>/dev/null || true)"
  if [ "$CHECK_ONLY_COMMAND_CHAIN" = "1" ]; then
  ...
  fail "gateway logs endpoint failed: $(cat "$TMP_DIR/oc-gateway-logs.err" 2>/dev/null || true)"
fi
...
if [ "$CHECK_ONLY_COMMAND_CHAIN" = "1" ]; then
  ...
  if api POST /api/openclaw/start >"$TMP_DIR/oc-start-2.json" 2>"$TMP_DIR/oc-start-2.err"; then
    ...
    awk -v m="$MARKER" 'f{print} $0~m{f=1}' /root/.openclaw/logs/gateway-watchdog.log 2>/dev/null | tail -n 120 >"$TMP_DIR/oc-t11-watchdog.log" || true
    if grep -q 'No such file or directory' "$TMP_DIR/oc-t11-watchdog.log" 2>/dev/null; then
      fail "watchdog still reports missing gateway log path"
    else
      pass "watchdog no missing-path errors after self-heal"
    fi
  else
    fail "restart api (self-heal case) failed: $(cat "$TMP_DIR/oc-start-2.err" 2>/dev/null || true)"
  fi
fi
