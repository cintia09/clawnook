...
5. **后端互斥锁已统一到 operation lock 域**：安装/更新/修复/重启均受 `/tmp/openclaw-operation.lock` 约束；原有任务锁仍保留用于同类任务复用。
...
- OpenClaw 源码目录：`/root/.openclaw/openclaw-source`
...
- Gateway 日志：`/root/.openclaw/logs/openclaw-gateway.log`
...
- Gateway 运行日志：`/root/.openclaw/logs/openclaw-gateway.log`（主路径）。
- 兼容旧路径：`/root/.openclaw/logs/gateway.log`。
- Watchdog 日志：`/root/.openclaw/logs/gateway-watchdog.log`。

实现约束（修正）：

1. watchdog 启动 gateway 前必须确保 `dirname(/root/.openclaw/logs/openclaw-gateway.log)` 存在；若创建失败，降级写入 legacy 路径并输出告警。
2. `/api/openclaw/gateway/logs` 默认仅返回一个 Gateway 主日志源（runtime 优先，legacy 兜底），避免 runtime/legacy 同时拼接导致“日志错位”。
3. 聚合输出需标记来源（`[gateway-runtime]` / `[gateway-legacy]` / `[watchdog]`），前端按快照去重显示。
4. Gateway 重启后前端应分阶段拉取日志快照（首次 + 延迟补拉），覆盖慢启动场景，避免“只有重启提示无启动细节”。
...
5. 新增统一操作锁：`/tmp/openclaw-operation.lock`，并输出结构化 `operationState`。
6. install/update/start/repair 共享同一锁域；同类任务可复用 `taskId`，跨类型任务返回冲突错误（409）。
