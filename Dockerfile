...
COPY scripts/openclaw-reset-password /usr/local/bin/
COPY post-install-restore.sh /opt/
COPY Caddyfile.template /etc/caddy/
RUN chmod +x /usr/local/bin/start-services.sh /usr/local/bin/openclaw-gateway-watchdog.sh /usr/local/bin/openclaw-reset-password /opt/post-install-restore.sh
