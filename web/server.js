...
function buildOpenClawSourceInstallCommand({ repo, tag, tarballUrl }) {
  ...
  return [
    ...
    'if ! curl -fL --http1.1 --retry 8 --retry-delay 2 --retry-all-errors -o "$TMP_BASE/openclaw.tar.gz" "$OPENCLAW_TARBALL_URL"; then',
    '  echo "[openclaw] codeload 下载失败，回退到 github archive..."',
    '  ALT_URL="https://github.com/$OPENCLAW_REPO/archive/refs/tags/$OPENCLAW_TAG.tar.gz"',
    '  curl -fL --http1.1 --retry 8 --retry-delay 2 --retry-all-errors -o "$TMP_BASE/openclaw.tar.gz" "$ALT_URL"',
    'fi',
    'mkdir -p "$SRC_TMP"',
