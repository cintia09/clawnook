...
(function () {
  ...
  const _en = {
    ...
    '：WhatsApp 通道插件尚未安装。请前往「': ': WhatsApp channel plugin is not installed. Go to "',
    '」安装 WhatsApp 插件后刷新此页面。': '" to install WhatsApp plugin, then refresh this page.',
    '：微信通道插件尚未安装。请前往「': ': WeChat channel plugin is not installed. Go to "',
    '」安装微信插件后刷新此页面。': '" to install WeChat plugin, then refresh this page.',
    '配置指引': 'Setup Guide',
    '：ClawBot 通过微信对话开放平台接入，在微信中以联系人形式出现。': ': ClawBot connects via WeChat Open Platform and appears as a contact in WeChat.',
    '启用后点击「获取登录二维码」，使用微信扫码完成绑定即可。': 'After enabling, click "Get QR Code" and scan with WeChat to complete binding.',
    '扫码登录': 'QR Code Login',
    '点击下方按钮获取二维码，使用微信扫一扫完成登录绑定。ClawBot 将作为联系人出现在你的微信中。': 'Click the button below to get a QR code, scan with WeChat to complete login. ClawBot will appear as a contact in your WeChat.',
    ...
    '）。': ').',
    '填入网关的 API 地址和密钥即可。': 'Enter the gateway API URL and key.',
    '插件市场 → Extensions': 'Plugin Market → Extensions',
    '微信对话开放平台': 'WeChat Open Platform',
    '可在「插件市场 → Extensions」手动安装': 'Can be installed manually from "Plugin Market → Extensions"',
