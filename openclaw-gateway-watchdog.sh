...
set -u
# Ensure watchdog runs in its own process group (isolate from PGID 1 group signals)
if [ "$(ps -o pgid= -p $$ 2>/dev/null | tr -d " ")" = "1" ] && command -v setsid >/dev/null 2>&1; then
    exec setsid -f "$0" "$@"
fi
...
# Signal handling: SIGTERM is NON-FATAL (ignored with log) to survive external
# process-group signals from WSL2/Docker. Use SIGUSR2 for intentional shutdown.
_watchdog_graceful_shutdown() {
  local sig="$1"
  log "[wd][signal] received SIG${sig} (pid=$$), performing graceful shutdown"
  rm -rf "$LOCK_DIR" >/dev/null 2>&1 || true
  exit 0
}
_watchdog_sigterm_handler() {
  # SIGTERM is non-fatal: log warning and keep running
  log "[wd][signal] SIGTERM received (pid=$$) — IGNORED (use SIGUSR2 to stop watchdog)"
}
...
trap '_watchdog_sigterm_handler' TERM
...
trap '_watchdog_graceful_shutdown USR2' USR2
